...
def get_option_chain():
    ...
    # Check Redis cache
    cached_data = create_app().redis_client.get(cache_key)
    if cached_data:
...
def calculate_t(scrip_id, segment):
    ...
    selected_expiry = create_app().redis_client.get(cache_key)
...
def background_task(redis_client, dhan_clients, instruments):
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        for dc, scrip_id, segment in zip(dhan_clients, instruments['scrip_id'], instruments['segment']):
